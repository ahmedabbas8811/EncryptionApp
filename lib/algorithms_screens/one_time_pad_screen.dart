import 'package:encryption_app/one_time_pad.dart';
import 'package:flutter/material.dart';

class OneTimePadScreen extends StatefulWidget {
  const OneTimePadScreen({super.key});

  @override
  State<OneTimePadScreen> createState() => _OneTimePadScreenState();
}

class _OneTimePadScreenState extends State<OneTimePadScreen> {
    ...
    try {
      setState(() {
        _resultText = _isEncrypting
            ? 'Encrypted: ${ClassicalOTP.encrypt(text, key)}'
            : 'Decrypted: ${ClassicalOTP.decrypt(text, key)}';
        _hasResult = true;
      });
    } catch (e) {
      setState(() {
  ...
  void _toggleMode() {
    setState(() {
      ...
      _textController.text = _resultText.replaceAll('Encrypted: ', '').replaceAll('Decrypted: ', '');
      _processText();
    });
  }
            ...
            children: [
              TextField(
                ...
                decoration: InputDecoration(
                  labelText: 'Enter Key (will auto-repeat/trim to match text)',
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                  filled: true,
                  fillColor: Colors.blueGrey.shade900.withOpacity(0.5),
                  prefixIcon: Icon(Icons.vpn_key, color: Colors.blueAccent.shade200),
                ),
              ),
              ...
              TextField(
                ...
                maxLines: 4,
                decoration: InputDecoration(
                  labelText: _isEncrypting ? 'Text to encrypt' : 'Binary to decrypt',
